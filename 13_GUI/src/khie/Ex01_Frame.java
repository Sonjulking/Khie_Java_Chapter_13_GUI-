 ...
 *   키보드와 마우스 등의 편리한 도구를 이용하여 사용자가 입력하기
 *   편하도록 만들어진 사용자 인터페이스임.
 * - java에서는 AWT와 Swing으로 어떠한 언어보다 강력한 GUI 
 *   라이브러리를 제공하여 소프트웨어 개발자들이 다양한 GUI를 쉽게
 *   구성할 수 있도록 해 주고 있음.
 *   
 * AWT와 Swing
 * - 자바 언어는 GUI 프로그램을 쉽게 작성할 수 있도록 다양한 GUI
 *   컴포넌트들을 제공함
 *   자바 컴포넌트는 AWT 컴포넌트와 Swing 컴포넌트로 구분이 됨.
 *   
 * 1. AWT 컴포넌트 : - java.awt 패키지를 통하여 제공이 됨.
 *                 - 운영체제의 도움을 받아 화면에 출력되기
 *                   때문에 자원을 많이 소모하여 운영체제에
 *                   많은 부담을 주게 됨.
 *                 - 운영체제마다 컴포넌트의 모양이 다르게 
 *                   나타남.
 ...
 *                   - 운영체제의 도움을 받지 않았음.
 *                     ==> 경량 컴포넌트라고 불림.
 *                   - AWT에는 없는 풍부한 고급 Swing 컴포넌트
 *                     들을 추가적으로 개발하였음.
 *                   - 다양하고 모양이 예쁜 Swing 컴포넌트들을
 *                     더 많이 사용함.
 *                     ==> 최근에는 거의 모든 GUI 프로그램이
 *                         Swing 기반으로 작성이 됨.
 */
 ...
 *             ==> 컴포넌트는 반드시 컨테이너에 올려져야
 *                 화면에 보여짐.
 * 3. 프레임 : 컨테이너들을 담을 수 있는 클래스.
 *            ==> 컨테이너는 반드시 프레임에 올려져야 화면에
 *                보여짐.
 */
